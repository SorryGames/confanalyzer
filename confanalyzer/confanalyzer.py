...
def _get_block(content, block_of, block_args):
    ...
    for index in range(len(content[0])):
        ...
        if block_counter and global_counter == 1 and block_of == "set":
            block_content.append(content[0][index])
        ...
        if block_counter == 1 and block_of == "vdom":
            if global_counter == 1:  # edit root
                block_content[-1] = "vdom" + block_content[-1][4:]
            if global_counter == 0:  # end
                block_content[-1] = "close" + block_content[-1][3:]
        #
        # config vdom
        # edit root
        # ...
        # end           <-- cause only one <end> closes config-edit vdom block
        if block_of == "vdom" and line == [ "config", "vdom" ]:
            global_counter -= 1
            next_line = _standard_form(content[0][index+1])
            if next_line == [ "edit" ] + block_args:
                block_counter += 1
        #
        if line[0] == block_end and block_counter: 
            block_counter -= 1
    #
    return (block_content, block_reference)
